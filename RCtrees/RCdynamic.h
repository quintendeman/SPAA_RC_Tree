...
void contract(cluster<T,D>* cluster_ptr)
{
    ...
    else if(cluster_ptr->get_neighbour_count() == 1)
    {
        ...
        other_side->state|=needs_adjustment;
    }
    else 
    if (cluster_ptr->get_neighbour_count() == 2)
    {
        ...
        left_node_ptr->state |= needs_adjustment;

        right_node_ptr->overwrite_neighbour(cluster_ptr, left_node_ptr, cluster_ptr);
        right_node_ptr->state |= needs_adjustment;
    ...
    frontier = parlay::flatten(
        parlay::delayed_tabulate(aff_ptrs.size(), [&] (T i) {
            ...
            if(ptr->alternate_adjacency.size() > 0)
            {
                ...
                for(short i = 0; i < w_node.size(); i+=2)
                {
                    auto& w_ptr = w_node[i];
                    ret_seq[i] = w_ptr;
                }
...
bool first_condition(cluster<T,D>* ptr, const unsigned char level)
{
    ...
    auto node_ptr = ptr->adjacency[level];
...
{      
    ...
    if(live_in_fip && live_in_fi)
    {
        ...
        for(short i = 0; i < ptr->adjacency[level]->size(); i+=2)
        {
            auto& w_ptr = (*ptr->adjacency[level])[i];
            if(w_ptr == nullptr)
                continue;
            // if w contracted in Fi, was it affected
            auto w_node_at_level = w_ptr->adjacency.get_tail();
            if(w_node_at_level->contraction_level == level)
                if((w_ptr->state & affected) == 0)
                    return false;
        }
        return true;
    }
...
{      
    ...
    auto& w_ptr_arr = ptr->alternate_adjacency[level]->adjacents;

    for(short i = 0; i < w_ptr_arr.size(); i+=2)
    {
        auto& w_ptr = w_ptr_arr[i];
        if(w_ptr == nullptr)
            continue;
        if((w_ptr->state & affected) == 0)
            if(w_ptr->adjacency.get_tail()->contraction_level == level)
                return false;
    }
...
void batchInsertEdge( const parlay::sequence<std::pair<T, T>>& delete_edges, const parlay::sequence<std::tuple<T, T, D>>& add_edges, parlay::sequence<cluster<T, D>>& clusters)
{
    ...
    auto aff_ptrs = parlay::filter(all_ptrs, [] (const auto& ptr){
        if(ptr != nullptr)
        {
            ...
            // revert ptr to state 0
            for(short i = 0; i < ptr->size; i+=2)
            {
                auto& w_ptr_arr = *ptr->adjacency[0]; 
                auto& w_alt_arr = *ptr->alternate_adjacency[0];    
                w_alt_arr[i] = ptr->ptrs[i] = w_ptr_arr[i];
                w_alt_arr[i+1] = ptr->ptrs[i+1] = w_ptr_arr[i+1];
                if(w_ptr_arr[i] == nullptr)
                {
                    ptr->types[i] = empty_type;
                    ptr->types[i+1] = empty_type;
                }
                else
                {
    ...
    parlay::parallel_for(0, delete_edges.size(), [&] (T i) {
        ...
        auto& w_ptr_arr = *clusters[v].adjacency[0];
        auto& w_alt_arr = *clusters[v].alternate_adjacency[0];
        for(short i = 0; i < clusters[v].size; i+=2)
        {
            if(w_ptr_arr[i] == &clusters[w])
            {
                clusters[v].types[i] = clusters[v].types[i+1] = empty_type;
                clusters[v].ptrs[i] = clusters[v].ptrs[i+1] = nullptr;
                w_alt_arr[i] = nullptr;
                w_alt_arr[i+1] = nullptr;
            }
            
        }
        auto& v_ptr_arr = *clusters[w].adjacency[0];
        auto& v_alt_arr = *clusters[w].alternate_adjacency[0];
        for(short i = 0; i < clusters[w].size; i+=2)
        {
            if(v_ptr_arr[i] == &clusters[v])
            {
                clusters[w].types[i] = clusters[w].types[i+1] = empty_type;
                clusters[w].ptrs[i] = clusters[w].ptrs[i+1] = nullptr;
                v_alt_arr[i] = nullptr;
                v_alt_arr[i+1] = nullptr;
            }
    ...
    do
    {
        ...
        parlay::parallel_for(0, reduce_edges.size(), [&] (T i) {
            ...
            if(clusters[v].counter == (i+1))
            {
                ...
                auto& w_ptr_arr = *(clusters[v].alternate_adjacency[0]);
                for(short ii = 0; ii < w_ptr_arr.size(); ii+=2)
                {
                    if(w_ptr_arr[ii] == nullptr)
                    {
                        w_ptr_arr[ii] = &clusters[w];
                        w_ptr_arr[ii+1] = newEdgePtr;
                        break;
                    }
                }
    ...
    do
    {
        ...
        parlay::parallel_for(0, add_edges_wflags.size(), [&] (T i) {
            ...
            if(clusters[w].counter == (i+1))
            {
                ...
                auto& v_ptr_arr = *(clusters[w].alternate_adjacency[0]);
                for(short ii = 0; ii < v_ptr_arr.size(); ii+=2)
                {
                    if(v_ptr_arr[ii] == nullptr)
                    {
                        v_ptr_arr[ii] = &clusters[v];
                        v_ptr_arr[ii+1] = new_edge;
                        break;
                    }
                }
    ...
    do
    {
        ...
        parlay::parallel_for(0, aff_ptrs.size(), [&] (T i){
            ...
            if(ptr->state & needs_adjustment)
            {
                ...
                std::array<cluster<T,D>*, max_size> new_arr(ptr->alternate_adjacency.get_tail()->adjacents); 

                for(short i = 0; i < new_arr.size(); i+=2)
                {
                    auto& other_ptr = new_arr[i];
                    if(other_ptr != nullptr && (other_ptr->contraction_time == count) && other_ptr->state & unary_cluster)
                    {
                        other_ptr = nullptr;
                        new_arr[i+1] = nullptr;
                    }
                }
                ptr->alternate_adjacency.add_level(new_arr, affected | live, count + 1);
                
                ptr->state |= affected;
                // ptr->state &= (~needs_adjustment);
            }
            return;
        });
        ...
        aff_ptrs = parlay::filter(aff_ptrs, [&] (auto ptr) {
            ...
            else if(ptr->state & affected)
            {
                if(ptr->alternate_adjacency.size() > 0)
                {
                    ptr->alternate_adjacency.add_level(ptr->alternate_adjacency.get_tail());
                    ptr->alternate_adjacency.get_tail()->contraction_level = count;
                }
                else
                {
                    ptr->alternate_adjacency.copy_till_level(&ptr->adjacency, count);
            ...
            return ptr->state & affected;
        });
        if(clusters.size() <= 100)
            printTree(clusters);
        std::cout << "\n\n\n\n\n" << std::endl;
        count++;
    }while(!(count > 1) && aff_ptrs.size() > 0);    
